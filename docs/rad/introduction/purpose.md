...
**Release 1.0** realises a self-contained demonstration of this experience on a single device: a user browses a catalog of mock trips and mock companion profiles, ranks them by a free-text search query, bookmarks favourites, maintains a local personal profile and privacy preferences, and can hold a **simulated conversation** with a companion through a local, keyword-based auto-reply engine. No other real user, server, or account is involved — every interaction is generated on-device from local mock data. Advanced search filters (by age, budget, location, etc.), real matching between distinct users, real-time messaging, and moderation are part of the long-term vision but are **not** realised in this lifecycle (see [1.2 Scope](./scope)).
